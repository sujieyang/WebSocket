...
class User(db.Model):
    ...
    email_hash = db.Column(db.String(100))  # 邮箱散列值
...
class Home(db.Model):
    ...
    home_name = db.Column(db.String(100))  # 房间名称
    home_topic = db.Column(db.String(50))  # 房间主题
    home_info = db.Column(db.String(500))  # 房间项目描述
...
class Now_count(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)  # 编号
    count = db.Column(db.Integer)  # 在线人数
